  ...
  const handleSend = async () => {
    ...
    try {
      if ((_a = window.electronAPI) == null ? void 0 : _a.sendToMCP) {
        const targetName = activeMCPs.length > 0 ? activeMCPs[0].name : "memory";
        const res = await window.electronAPI.sendToMCP({ name: targetName, prompt });
        setResponse(res);
      } else {
